...
def calculate_pagerank(urls, links, convergence_threshold, max_iterations, damping_factor=0.85):
    ...
    count = 0
    with open("pagerank_work.output", "w") as file:
        file.write("0.25 for all 0th iterations\n\n")
        ...
        while count < max_iterations:
            ...
            for url in urls:
                ...
                pagerank[url] = round((1 - damping_factor) / N + damping_factor * predecessor_sum_score, 3)
                    
                file.write(f"] * {damping_factor} + (1 - {damping_factor}) / {N} = {round(pagerank[url], 3)}\n")
